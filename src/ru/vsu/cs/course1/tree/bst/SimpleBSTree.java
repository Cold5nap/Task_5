...
public class SimpleBSTree<T extends Comparable<? super T>> extends BinaryTree<T> implements DefaultBSTree<T> {
     ...
     */
    public SimpleTreeNode getMinNodeParent(SimpleTreeNode node) {
        if (node == null) {
            return null;
        }
    ...
    /**
     * Возращает меньше указанного или равное значение узла
     * в SimpleBSTree,если такое имеется. Если такого чиисла нет, вернет NULL.
     *
     * @param indicated указанное число
     * @return равное или меньшее число, null
     */
    public T getLessOrEqual(T indicated) {
        TreeNode<T> node = root;
        boolean isChangeWay = true;
        T lastValue = null;

        while (isChangeWay) {
            isChangeWay = false;
            if (node.getValue().compareTo(indicated) > 0) {

                while (node.getValue().compareTo(indicated) > 0
                        && node.getLeft() != null) {
                    node = node.getLeft();
                    isChangeWay = true;
                }
        ...
        return node.getValue().compareTo(indicated) > 0 ? lastValue : node.getValue();
    }

    /**
     * Возращает больше указанного или равное значение узла
     * в SimpleBSTree,если такое имеется. Если такого чиисла нет, вернет NULL.
     *
     * @param indicated указанное число
     * @return равное, или большее число, или null
     */
    public T getGreaterOrEqual(T indicated) {
        TreeNode<T> node = root;
        boolean isChangeWay = true;
        T lastValue = null;
    ...
    /**
     * Возращает максимальное значение в SimpleBSTree
     *
     * @return maxValue
     */
    @Override
    public T getMax() {
        DefaultBinaryTree.TreeNode<T> maxNode = getMaxNode();
        return (maxNode == null) ? null : maxNode.getValue();
    }

    /**
     * Возращает минимальное значение имеющееся в дереве SimpleBSTree
     *
     * @return minValue
     */
    @Override
    public T getMin() {
        DefaultBinaryTree.TreeNode<T> maxNode = getMinNode();
        return (maxNode == null) ? null : maxNode.getValue();
    }
